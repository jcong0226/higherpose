...
class SuperLitePose(nn.Module):
    def __init__(self, cfg, width_mult=1.0, round_nearest=8):
        ...
        self.final_refined, self.final_raw, self.final_channel = self._make_final_layers(
                cfg, extra.NUM_DECONV_FILTERS, cfg.MODEL.NUM_JOINTS)
    ...
    def forward(self, x):  # torch.Size([4, 3, 256, 256])
        ...
        input_refined = self.global_encoder(input_refined, h, w, pos=self.pos_embedding)  # torch.Size([256, 4, 80])
        ...
        for i in range(self.num_deconv_layers):  # 3
            ...
            if i > 0:
                final_refined = self.final_refined[i - 1](input_refined, self.final_channel[i - 1])  # torch.Size([4, 10, 64, 64]) torch.Size([4, 5, 128, 128])
                final_raw = self.final_raw[i - 1](input_raw, self.final_channel[i - 1])  # torch.Size([4, 10, 64, 64]) torch.Size([4, 5, 128, 128])
