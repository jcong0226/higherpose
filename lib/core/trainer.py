...
def do_train(cfg, model, lr_scheduler, data_loader, loss_factory, optimizer, epoch,
    ...
    for i, (images, heatmaps, masks, joints) in enumerate(data_loader):
        ...
        if i % cfg.PRINT_FREQ == 0 and cfg.RANK == 0:
            msg_name = '| Epoch \t\t\t' + '| Time \t\t\t\t\t' + '| Speed \t\t\t' + '| Data \t\t\t\t\t' + \
                  '| Stage0-heatmaps \t\t' + '| Stage1-heatmaps \t\t\t' + \
                  '| Stage0-push \t\t\t' + '| Stage1-push \t\t\t\t' + '| Stage0-pull \t\t\t' + '| Stage1-pull \t\t\t\t|\n'
            msg_value = '| [{0}][{1}/{2}]\t\t' \
                  '| {batch_time.val:.3f}s ({batch_time.avg:.3f}s)\t\t' \
                  '| {speed:.1f} samples/s\t\t' \
                  '| {data_time.val:.3f}s ({data_time.avg:.3f}s)\t\t' \
            ...
            logger.info(msg_name+msg_value)
            ...
            #     )
    return heatmaps_loss_meter[0].sum, push_loss_meter[0].sum, pull_loss_meter[0].sum


def _get_loss_info(loss_meters, loss_name):
    msg = ''
    for i, meter in enumerate(loss_meters):
        msg += '| {meter.val:.3e} ({meter.avg:.3e})\t'.format(
